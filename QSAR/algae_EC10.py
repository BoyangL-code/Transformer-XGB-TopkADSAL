...
# ========== 数据读取 ==========
train_data = pd.read_excel("./algae_EC10_train.xlsx")
train_data = train_data.dropna().copy()

test_data = pd.read_excel("./algae_EC10_test.xlsx")
...
# ========== 超参数 ==========
best_params = {
    'dropout_rate': 0.19169772724006684,
    'fc1_size': 960,
    'fc2_size': 160,
    'fc3_size': 160,
    'learning_rate': 1.500330592476472e-05,
    'weight_decay': 0.021610626849986693
}
...
torch.save(model.state_dict(), "./saved_models/algae_EC10_train_test_split.pth")
print("✅ 训练完成，模型已保存到 ./saved_models/algae_EC10_train_test_split.pth")
...
os.makedirs('./algae_EC10/embeddings', exist_ok=True)
os.makedirs('./algae_EC10/bert_prediction', exist_ok=True)

np.save('./algae_EC10/embeddings/train_single.npy', X_train)
np.save('./algae_EC10/embeddings/test_single.npy', X_test)
np.save('./algae_EC10/embeddings/train_labels_single.npy', labels_train)
np.save('./algae_EC10/embeddings/test_labels_single.npy', labels_test)
...
np.save('./algae_EC10/bert_prediction/bert_single_test_predictions.npy', y_preds)

print("✅ 嵌入、标签与测试集预测已保存（train/test split 版本）。")

data_dir = './algae_EC10/embeddings'
save_dir = './algae_EC10/xgb_predictions'
...
best_params = {
    'n_estimators': 800, 
    'learning_rate': 0.013184384270236356, 
    'max_depth': 3, 
    'subsample': 0.9151113240783383, 
    'colsample_bytree': 0.9433652849644489
}
